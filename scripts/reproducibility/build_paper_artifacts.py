...
def build(root: Path, output_dir: Path, overwrite: bool) -> None:
    ...
    if overwrite:
        unknown = [
            ...
            and path.name not in RETIRED_FILENAMES
        ...
        for name in BUNDLE_FILENAMES | RETIRED_FILENAMES:
    ...
    shared_generation_config = load_experiment_config(
        require(root / f"configs/expanded/{experiment_names[0]}.json")
    )
    ...
    for experiment_name in experiment_names:
        ...
        for key in ("eval_seeds", "generator", "generation"):
            if experiment_config[key] != shared_generation_config[key]:
                raise ValueError(f"Generation setting {key!r} differs across groups")
            ...
            target_rows.append(
                {
                    ...
                    "forget_id": forget_id,
                    "policy": policy,
                    "target_identity": int(target["target_identity"]),
                    "target_similarity_to_forget": float(
                        target["target_similarity_to_forget"]
                    ),
                }
            )
    ...
    if set(adapter_ranks) != {8}:
        raise ValueError("Expected every selected adapter to have rank 8")
    np.savez_compressed(
    ...
    protocol = {
        ...
        "generation_protocol": {
            "adapter_rank": 8,
            "eval_seeds": shared_generation_config["eval_seeds"],
            "generator": shared_generation_config["generator"],
            "generation": shared_generation_config["generation"],
        },
