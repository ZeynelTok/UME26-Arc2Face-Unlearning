...
def main() -> None:
    ...
    forget_embeddings = _normalize_matrix(
        np.stack([reference_face_embedding(embedder, path) for path in forget_paths], axis=0)
    )
    forget_centroid = centroid(forget_embeddings)

    retain_paths = _collect_retain_paths(fit_reference_images, config)
    retain_embeddings = _normalize_matrix(
        np.stack([reference_face_embedding(embedder, path) for path in retain_paths], axis=0)
    )
