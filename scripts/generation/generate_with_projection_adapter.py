...
from genmu_face_unlearning.face_embedding import reference_face_embedding
from genmu_face_unlearning.generation import build_identity_plan, save_generated_images, write_manifest
...
def main() -> None:
    ...
    for identity, role in build_identity_plan(config):
        for reference_path in group_reference_list(group, identity, split="fit"):
            embedding = reference_face_embedding(embedder, reference_path)
            projected, cosine_to_forget, residual_norm = apply_projection_adapter(embedding, adapter)
            records = save_generated_images(
                runtime=runtime,
                out_dir=args.out_dir,
                role=role,
                identity=identity,
                reference_path=reference_path,
                face_embedding=projected,
                seeds=config["eval_seeds"],
                settings=config["generation"],
            )
            for record in records:
                record["cosine_to_forget_centroid"] = cosine_to_forget
                record["projection_residual_norm"] = residual_norm
